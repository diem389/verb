...
verb_core_KdTree.prototype = {
	...
	,nearest: function(point,maxNodes,maxDistance) {
		...
		var nearestSearch1 = nearestSearch;
		var _g12 = 0;
		var _g3 = maxNodes;
		while(_g12 < _g3) {
			var i3 = _g12++;
...
var verb_core_LazySurfaceBoundingBoxTree = function(surface,splitV,knotTolU,knotTolV) {
	...
	if(knotTolV == null) {
		var a1 = surface.knotsV;
		knotTolV = (verb_core_ArrayExtensions.last(a1) - verb_core_ArrayExtensions.first(a1)) / 16;
	}
...
verb_core_LazySurfaceBoundingBoxTree.prototype = {
	...
	,indivisible: function(tolerance) {
		...
		if(verb_core_ArrayExtensions.last(a) - verb_core_ArrayExtensions.first(a) < this._knotTolV) {
			var a1 = this._surface.knotsU;
			return verb_core_ArrayExtensions.last(a1) - verb_core_ArrayExtensions.first(a1) < this._knotTolU;
		} else {
			return false;
		}
	}
...
verb_eval_Analyze.rationalSurfaceClosestParam = function(surface,p) {
	...
	var n = function(uv1,e1,r) {
		...
		var d1 = verb_core_Mat.solve(J,k);
		return verb_core_Vec.add(d1,uv1);
	};
	while(i < maxits) {
		e = f(cuv);
...
verb_eval_Analyze.rationalCurveClosestParam = function(curve,p) {
	...
	var cu = u;
	var f = function(u2) {
		return verb_eval_Eval.rationalCurveDerivatives(curve,u2,2);
	};
	var n = function(u3,e1,d1) {
		var f1 = verb_core_Vec.dot(e1[1],d1);
		var s0 = verb_core_Vec.dot(e1[2],d1);
...
verb_eval_Intersect.curves = function(curve1,curve2,tolerance) {
	...
	return verb_core_ArrayExtensions.unique(ints.map(function(x) {
		var tmp = verb_core_ArrayExtensions.first(x.item0.knots);
		var tmp1 = verb_core_ArrayExtensions.first(x.item1.knots);
		return verb_eval_Intersect.curvesWithEstimate(curve1,curve2,tmp,tmp1,tolerance);
	}).filter(function(x1) {
...
verb_eval_Make.surfaceIsocurve = function(surface,u,useV) {
	...
	if(useV) {
		...
		return new verb_core_NurbsCurveData(newSrf1,newSrf2,_g2);
	}
...
verb_eval_Make.loftedSurface = function(curves,degreeV) {
	...
	while(_g1 < _g) {
		var i = [_g1++];
		var points = curves.map((function(i1) {
			return function(x) {
				return x.controlPoints[i1[0]];
			};
		})(i));
...
verb_eval_Make.revolvedSurface = function(profile,center,axis,theta) {
	...
	while(_g11 < _g3) {
		...
		while(_g31 < _g21) {
			var i2 = _g31++;
			var P2 = r == 0 ? O : verb_core_Vec.add(O,verb_core_Vec.add(verb_core_Vec.mul(r * cosines[i2],X),verb_core_Vec.mul(r * sines[i2],Y)));
...
verb_eval_Make.rationalInterpCurve = function(points,degree,homogeneousPoints,start_tangent,end_tangent) {
	...
	while(_g13 < _g5) {
		...
		var x1 = verb_core_Mat.solve(A,b);
		xs.push(x1);
	}
...
verb_eval_Modify.unifyCurveKnotVectors = function(curves) {
	...
	var knotIntervals = _g2;
	var _g21 = 0;
	var _g12 = curves.length;
	while(_g21 < _g12) {
		var i1 = _g21++;
		var min = [knotIntervals[i1].min];
		var curves1 = curves[i1].knots;
		var tmp = (function(min1) {
			return function(x1) {
				return x1 - min1[0];
			};
		})(min);
		curves[i1].knots = curves1.map(tmp);
	}
	...
	while(_g22 < _g13) {
		...
		var curves2 = curves[i2].knots;
		var tmp1 = (function(scale1) {
			return function(x4) {
				return x4 * scale1[0];
			};
		})(scale);
		curves[i2].knots = curves2.map(tmp1);
	}
	var mergedKnots = Lambda.fold(curves,function(x5,a2) {
		return verb_core_Vec.sortedSetUnion(x5.knots,a2);
	},[]);
	var _g23 = 0;
...
verb_eval_Modify.curveElevateDegree = function(curve,finalDegree) {
	...
	while(_g11 < _g4) {
		...
		var _g31 = verb_eval_Modify.imax(0,i1 - degreeInc);
		var _g21 = mpi1 + 1;
		while(_g31 < _g21) {
			var j1 = _g31++;
	...
	while(b < m) {
		...
		while(_g15 < _g8) {
			...
			var _g32 = verb_eval_Modify.imax(0,i5 - degreeInc);
			var _g22 = mpi2 + 1;
			while(_g32 < _g22) {
				var j3 = _g32++;
	...
	while(j >= 0) {
		...
		while(_g14 < _g5) {
			...
			if(Math.abs(alfa) < verb_core_Constants.EPSILON) {
				...
				alfa /= knots_post[k + l] - knots[i4 - degree + l];
...
verb_eval_AdaptiveRefinementNode.prototype = {
	...
	,getAllCorners: function(edgeIndex) {
		...
		var rangeFuncMap = [function(c) {
			...
			if(c1.uv[1] > that.corners[0].uv[1] + e) {
				return c1.uv[1] < that.corners[2].uv[1] - e;
			} else {
				return false;
			}
		}];
...
verb_exe_WorkerPool.prototype = {
	...
	,processQueue: function() {
		...
		while(this._queue.length > 0 && this._pool.length > 0) {
			...
			worker[0].onmessage = (function(worker1,workId1) {
				return function(e) {
					_gthis._working.remove(workId1[0]);
					_gthis._pool.push(worker1[0]);
					try {
						if(_gthis._callbacks.h.hasOwnProperty(workId1[0])) {
							_gthis._callbacks.h[workId1[0]](e.data.result);
							_gthis._callbacks.remove(workId1[0]);
						}
					} catch( error ) {
